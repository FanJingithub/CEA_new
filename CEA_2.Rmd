...
那么CEA与复发风险之间的关系究竟如何。使用RCS策略处理连续性变量CEA，发现CEA=5是一个重要的拐点，大于5和小于5时，CEA与log(HR)都呈线性关系，只是斜率不同（如下图所示）。考虑将CEA分成两段处理，重新建模，C-index有所提升，AUC表现也有改善（超过0.75）。
```{r}
P2
```
...
### 各时点AUC
此处将分别展示1、3、5年的AUC，发现3年和5年时刻的AUC明显很高，模型预测能力很好。
...
将模型的预测风险评分进行从小到大排序，展示各个病例对应的复发时间和是否出现复发、T分期、N分期、CEA情况
...
<img src="D:/data_work/CEA/plots/1-year DFS_training.png" style="width:600px;height:600px">

预测3年生存率的校准曲线：

<img src="D:/data_work/CEA/plots/3-year DFS_training.png" style="width:600px;height:600px">

预测5年生存率的校准曲线：

<img src="D:/data_work/CEA/plots/5-year DFS_training.png" style="width:600px;height:600px">
